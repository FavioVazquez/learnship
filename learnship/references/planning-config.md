...
| `workflow.discuss_mode` | `"discuss"` | Discussion depth for `discuss-phase` and `new-project` questioning. `"discuss"` = standard (4 focused exchanges). `"deep"` = grill-me style, walks every decision branch until shared understanding — equivalent to passing `--deep` flag |
