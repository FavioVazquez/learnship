...
- **Deep questioning and vertical slice planning.** v2.3.4 adds `--deep` extended questioning for `/discuss-phase` and `/new-project` (walks every decision branch until shared understanding), and enforces vertical slice planning in `/plan-phase` (each plan is a tracer bullet through all integration layers; horizontal plans are flagged).
- **Two-stage review, OWASP Top 10, and live UI smoke tests.** v2.4.0 splits `/review` into spec compliance (did we build what was planned?) then quality (is it well-built?). `/secure-phase` now cross-references STRIDE findings against OWASP Top 10 (A01–A10) for exhaustive audit coverage. `/health` now outputs a numeric 0–100 score with HEALTHY/DEGRADED/BROKEN bands. `/verify-work` and `/ship` both gain optional Playwright MCP live smoke-test sections that work on any MCP-enabled platform.
